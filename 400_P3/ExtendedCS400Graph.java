...
public class ExtendedCS400Graph<NodeType,EdgeType extends Number> implements ExtendedGraphADT<NodeType,EdgeType> {
    ...
    public ExtendedCS400Graph() { vertices = new Hashtable<>(); }
    ...
    public boolean insertEdge(NodeType source, NodeType target, EdgeType weight) {
        ...
        Vertex sourceVertexOpp = this.vertices.get(target);
        Vertex targetVertexOpp = this.vertices.get(source);
        // handle cases where edge already exists between these verticies
        for(Edge e : sourceVertexOpp.edgesLeaving)
            if(e.target == targetVertexOpp) {
                if(e.weight.doubleValue() == weight.doubleValue()) return false; // edge already exists
                else e.weight = weight; // otherwise update weight of existing edge
                return true;
            }
        // otherwise add new edge to sourceVertexOpp
        sourceVertexOpp.edgesLeaving.add(new Edge(targetVertexOpp,weight));
        return true;
    }    
    ...
    public boolean removeEdge(NodeType source, NodeType target) {
        ...
        Vertex targetVertexOpp = this.vertices.get(source);
        // find edge to remove
        Edge removeEdge2 = null;
        for(Edge e : sourceVertexOpp.edgesLeaving)
            if(e.target == targetVertexOpp) 
                removeEdge2 = e;
        if(removeEdge2 != null) { // remove edge that is successfully found                
            sourceVertexOpp.edgesLeaving.remove(removeEdge2);
            return true;
        }
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
    	...
    	while(frontier.size()!=0) {
    		if(frontier.peek().end.equals(vertices.get(end))) {
    			Path path = frontier.remove();
    			return path;
    		}
    		else {
    			Path path = frontier.remove();
    			for(int i=0;i<path.end.edgesLeaving.size();i++) { //known path
    				Path temp = new Path(path, path.end.edgesLeaving.get(i));
    				boolean bool = true;
    				for(int j=0;j<path.dataSequence.size();j++) { // extended path
    					if(path.dataSequence.get(j).equals(path.end.edgesLeaving.get(i).target.data))
    						bool = false;
    				}	
    				if(bool)
    					frontier.add(temp);
